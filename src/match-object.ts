...
import { clientEnvelopeSchema, type LobbyPayload, type ServerEnvelope } from './envelope'
 ...
 */
export interface MatchObjectApi<S> {
...
export type MatchObjectClass<S> = new (ctx: DurableObjectState, env: MatchEnv) => MatchObjectApi<S> & DurableObject<MatchEnv>
...
export function createMatchObject<S, I, V, E>(adapter: GameAdapter<S, I, V, E>): MatchObjectClass<S> {
  return class MatchObject extends DurableObject<MatchEnv> implements MatchObjectApi<S> {
    ...
    send(ws: WebSocket, msg: ServerEnvelope): void {
      try {
    ...
    sendToSeat(seat: number, msg: ServerEnvelope): void {
    ...
    handleHello(ws: WebSocket, hello: { token?: string; seatToken?: string; displayName?: string }): void {
      const meta = this.getMeta()
    ...
    handleConfigure(seat: number, players: number, bots: number): void {
      const meta = this.getMeta()!
      if (seat !== 0)
    ...
    seatSocket(ws: WebSocket, seat: number, token: string, rejoin: boolean): void {
    ...
    handleIntent(seat: number, raw: unknown): void {
    ...
    applyAndBroadcast(intent: I & { player: number }, errorTo?: number): void {
      const meta = this.getMeta()!
     ...
     */
    driveMemory(seat: number): unknown {
    ...
    nextDrivenSeat(state: S): { seat: number; kind: DriveKind } | null {
      for (const row of this.seats()) {
    ...
    async fireAbandon(): Promise<void> {
      this.setDeadline('abandon', null)
      const meta = this.getMeta()
      const state = this.loadState()
